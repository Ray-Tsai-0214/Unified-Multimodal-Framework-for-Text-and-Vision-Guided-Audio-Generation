...
from ldm.modules.encoders.ImageBind_AD.combine import get_adapter_conditioning
...
class FrozenCLAPEmbedder(AbstractEncoder):
    ...
    def encode(self, text):
        batch_encoding = self.tokenizer(text, truncation=True, max_length=self.max_length, return_length=True,
                                        return_overflowing_tokens=False, padding="max_length", return_tensors="pt")
        tokens = batch_encoding["input_ids"].to(self.device) ##[1,77]

        outputs = self.caption_encoder.base(input_ids=tokens)
        z = self.caption_encoder.projection(outputs.last_hidden_state)
        #z = get_adapter_conditioning(text_prompt=text, image_path='data/.assets/car_image.jpg')  # → [1, 77, 1024]
