...
class SpectrogramDataModuleFromConfig(DataModuleFromConfig):
    ...
    def __init__(self, batch_size, num_workers,spec_dir_path=None,main_spec_dir_path=None,other_spec_dir_path=None,
                 ...
                 require_caption=True, embedding_dir_path=None, train=None, validation=None, test=None, predict=None, wrap=False):
        specs_dataset_cfg = {
    ...
    def test_dataloader(self):
        init_fn = None
        return DataLoader(self.datasets["test"], batch_size=self.batch_size, num_workers=self.num_workers, worker_init_fn=init_fn, shuffle=False)
...
if __name__ == "__main__":
    ...
    try:
        ...
        # default to ddp on Linux/Mac, but use dp on Windows
        import platform
        if platform.system() == "Windows":
            trainer_config["strategy"] = "dp"  # Use DataParallel on Windows
        else:
            trainer_config["strategy"] = "ddp"  # Use DDP on Linux/Mac
        ...
        if not "gpus" in trainer_config:
            ...
            print(f"Using strategy: {trainer_config.get('strategy', 'auto')}")
            cpu = False
        ...
        if not cpu:
            if isinstance(lightning_config.trainer.gpus, int):
                ngpu = 1 if lightning_config.trainer.gpus == 0 else lightning_config.trainer.gpus
            else:
        ...
        if platform.system() != 'Windows':
            ...
            print("Note: Signal handlers (SIGUSR1/SIGUSR2) are not available on Windows")
