...
class LatentDiffusion_audio(DDPM):
    ...
    def __init__(self,
        ...
        # 重要：將 freeze_model 儲存但不傳給父類
        self.freeze_model_flag = freeze_model
    ...
    def freeze_unet(self):
        ...
        print(f"Froze UNet model with {count_params(self.model)*1e-6:.2f}M parameters")
        
    def unfreeze_unet(self):
        """解凍 UNet 模型參數"""
        for param in self.model.parameters():
            param.requires_grad = True
        print(f"Unfroze UNet model with {count_params(self.model)*1e-6:.2f}M parameters")
    ...
    def get_input(self, batch, k, return_first_stage_outputs=False, force_c_encode=False,
        ...
        if self.model.conditioning_key is not None:
            ...
            if cond_key != self.first_stage_key:
                if cond_key in ['caption', 'coordinates_bbox', 'embedding']:
                    xc = batch[cond_key]
    ...
    def lr_scheduler_step(self, scheduler, optimizer_idx, metric):
        # Override to handle custom LambdaLR scheduler
        scheduler.step()
