...
from .embedding_encoder import EmbeddingConditioner

class FrozenEmbeddingConditioner(AbstractEncoder):
    """使用預計算 embeddings 的凍結條件編碼器"""
    def __init__(self, **kwargs):
        super().__init__()
        self.conditioner = EmbeddingConditioner(**kwargs)
        self.conditioner.freeze()
    
    def encode(self, embedding):
        return self.conditioner.encode(embedding)

class TrainableEmbeddingConditioner(AbstractEncoder):
    """可訓練的 embedding 條件編碼器"""
    def __init__(self, **kwargs):
        super().__init__()
        self.conditioner = EmbeddingConditioner(**kwargs)
